struct Profile : JSONDecodable {
    ...
    init(json: Any) throws {
        ...
        guard let email = dictionary["email"] as? String else {
            throw JSONDecodeError.missingValue(key: "email", actualValue: dictionary["email"])
        }

        guard let notificationsCount = dictionary["notifications_count"] as? Int else {
            throw JSONDecodeError.missingValue(key: "notifications_count", actualValue: dictionary["notifications_count"])
        }
